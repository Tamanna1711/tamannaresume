	...
	function() {
		...
		function shift_cv_resize_video(cont) {
			...
			cont.find( 'video' ).each(
				function() {
					// If item now invisible
					if (jQuery( this ).hasClass( 'trx_addons_resize' ) || jQuery( this ).addClass( 'shift_cv_resize' ).parents( 'div:hidden,section:hidden,article:hidden' ).length > 0) {
						return;
					}
			...
			// If item now invisible
			if (jQuery(this).addClass('shift_cv_resize').parents('div:hidden,section:hidden,article:hidden').length > 0) {
				return;
			}
		...
		function shift_cv_init_media_elements(cont) {
			if (SHIFT_CV_STORAGE['use_mediaelements'] && cont.find( 'audio:not(.inited),video:not(.inited)' ).length > 0) {
				if (window.mejs) {
