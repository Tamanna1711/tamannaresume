...
if ( ! function_exists( 'shift_cv_importer_set_options' ) ) {
	...
	function shift_cv_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			$options['files']['default']['domain_dev']  = esc_url( 'http://shiftcv.dv.themerex.net' );       // Developers domain
			$options['files']['default']['domain_demo'] = esc_url(  'http://'. $rtl_url .'shift-cv.themerex.net' ); 								// Demo-site domain
