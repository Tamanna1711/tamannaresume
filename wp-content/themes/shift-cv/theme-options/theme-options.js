...
jQuery( document ).ready(
	function() {
		...
		function shift_cv_options_check_dependencies(cont) {
			if (typeof shift_cv_dependencies == 'undefined') return;
			cont.find( '.shift_cv_options_item_field,.shift_cv_options_group[data-param]' ).each(
				function() {
					...
					var depend = false;
					for (var fld in shift_cv_dependencies) {
						if (fld == id) {
					...
					if (depend) {
						...
						for (i in depend) {
							...
							if (name.charAt( 0 ) == '#' || name.charAt( 0 ) == '.') {
								fld = jQuery( name );
								if (fld.length > 0 && ! fld.hasClass( 'shift_cv_inited' )) {
									fld.addClass( 'shift_cv_inited' ).on(
										'change', function () {
											jQuery( '.shift_cv_options .shift_cv_options_section' ).each(
												function () {
													shift_cv_options_check_dependencies( jQuery( this ) );
						...
						if (((dep_cnt > 0 || dep_all == 0) && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
							ctrl.show().removeClass( 'shift_cv_options_no_use' );
							...
							ctrl.hide().addClass( 'shift_cv_options_no_use' );
							if ( section.find('>.shift_cv_options_item:not(.shift_cv_options_item_info),>.shift_cv_options_group[data-param]').length == section.find('.shift_cv_options_no_use').length ) {
								if ( ! tab.hasClass( 'shift_cv_options_item_hidden' ) ) {
