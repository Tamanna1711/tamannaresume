...
// Add images and icons to categories (old way, See extended_taxonomy)
//--------------------------------------------------------------------------
...
// Return small image (icon) from the category  (old way, See extended_taxonomy)
	...
	function trx_addons_get_current_mode_image($default='') {
		if (($img = apply_filters('trx_addons_filter_get_current_mode_image', $default)) != '') {
			...
			if (is_category() || is_tax()) {
				if (($img = trx_addons_get_taxonomy_image()) != '') {
					$default = $img;
				}
			} else if (is_singular()) {
